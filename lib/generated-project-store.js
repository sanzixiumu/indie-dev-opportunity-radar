...
function getProjectCreatedAtTime(project) {
...
function sortNewestFirst(projects) {
  return projects
    .slice()
    .sort((a, b) => getProjectCreatedAtTime(b) - getProjectCreatedAtTime(a));
}

function validateProject(project) {
  if (!project || !project.id) {
    throw new Error("Generated project must include project.id.");
  }
}
...
function createGeneratedProjectStore(storage) {
  ...
  function getProjects() {
    return sortNewestFirst(
      normalizeProjects(resolvedStorage.getStorageSync(STORAGE_KEY)),
    );
  }
