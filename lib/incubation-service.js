...
function copyAnswer(answer) {
  return Object.assign({}, answer, {
    selectedOptions: (answer.selectedOptions || []).slice(),
  });
}
...
function createIncubationSession(idea) {
  ...
  return {
    idea: normalizedIdea,
    questions: createQuestionsForIdea(normalizedIdea),
    answers: [],
...
function getCurrentQuestion(session) {
  return copyQuestion(session.questions[session.currentQuestionIndex]);
}
...
function answerCurrentQuestion(session, answer) {
  ...
  return Object.assign({}, session, {
    questions: copyQuestions(session.questions),
    answers: session.answers.map(copyAnswer).concat(nextAnswer),
...
function goToPreviousQuestion(session) {
  ...
  return Object.assign({}, session, {
    questions: copyQuestions(session.questions),
    answers: session.answers.slice(0, -1).map(copyAnswer),
    currentQuestionIndex: session.currentQuestionIndex - 1,
  });
}
...
function generateProjectResult(session) {
  ...
  const slug = Date.now() + "-" + ++projectIdCounter;

  return {
    id: "project-" + slug,
    sourceIdea: session.idea,
    answers: session.answers.map(copyAnswer),
    title: "AI 驱动的细分场景效率工具",
    conclusion:
      "建议做，但需要从明确人群和高频任务切入，避免一开始做成泛用平台。",
    domesticProducts: [
      createProduct(
        "扣子",
        "AI Bot 与工作流搭建平台",
    ...
    entryDirection:
      "先做一个聚焦单一人群的轻量决策助手，用 AI 把输入想法转成可验证的行动清单。",
    advantages: [
      "可以避开大平台的通用能力竞争",
      "小程序形态适合低门槛分享",
      "适合用内容渠道获取种子用户",
